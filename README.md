...
> Answer: When using client-side routing, we do not have to continue to go to the browser for a full reload every time the information changes. Client side routing allows us the feel of a whole new page, while really only reloading the individual components locally on the machine. This makes for a much faster and pleasant user interface.
